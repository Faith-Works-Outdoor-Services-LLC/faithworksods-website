...
- Estimate forms include city/job location, photo-texting prompt, best contact time, and access notes.

## Still needs real account values before paid launch
1. Facebook URL - replace `PLACEHOLDER` in `website/_build_site.py`.
2. YouTube URL - replace `PLACEHOLDER` in `website/_build_site.py`.
3. Google Business Profile URL - replace `PLACEHOLDER` after GBP verification.
4. Formspree form ID - save to `formspree-id.txt` or set `FORMSPREE_FORM_ID`.
